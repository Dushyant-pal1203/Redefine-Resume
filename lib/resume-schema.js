...
export function getEmptyResumeData() {
  return {
    ...
    skills: {
      technical: [],
      soft: [],
      languages: [], // For programming languages or technical skills
    },
    projects: [],
    certifications: [],
    languages: [], // For spoken languages (English, Hindi, etc.)
    publications: [],
    awards: [],
    volunteering: [],
    interests: [],
...
export function convertToFlexibleFormat(oldData) {
  ...
  // Process skills separately from languages
  let technicalSkills = [];
  let softSkills = [];
  let programmingLanguages = []; // For skills.languages
  let spokenLanguages = []; // For top-level languages

  // Handle skills from old format
  if (Array.isArray(oldData.skills)) {
    // Try to categorize skills (simple heuristic)
    ...
    oldData.skills.forEach((skill) => {
      const skillLower = String(skill).toLowerCase();
      if (softSkillKeywords.some((keyword) => skillLower.includes(keyword))) {
        softSkills.push(skill);
      } else if (
        programmingKeywords.some((keyword) => skillLower.includes(keyword))
      ) {
    ...
    technicalSkills = oldData.technical_skills || [];
    softSkills = oldData.soft_skills || [];
    programmingLanguages = oldData.programming_languages || [];
  }

  // Handle spoken languages from old format
  if (Array.isArray(oldData.languages)) {
    spokenLanguages = oldData.languages.map((lang) =>
      typeof lang === "string"
  ...
  return {
    ...
    skills: {
      ...
      soft: softSkills,
      languages: programmingLanguages, // Programming languages
    },
    ...
    certifications: Array.isArray(oldData.certifications)
      ...
      : [],
    languages: spokenLanguages, // Spoken languages
...
export function convertDatabaseToFlexibleFormat(dbData) {
  ...
  if (Array.isArray(dbData.skills)) {
    programmingLanguages = dbData.skills;
  }

  if (Array.isArray(dbData.languages)) {
    spokenLanguages = dbData.languages.map((l) =>
      typeof l === "string" ? { language: l, proficiency: "Professional" } : l,
    );
  }
  ...
  return {
    ...
    education: Array.isArray(dbData.education)
      ...
      : [],
    skills: {
      technical: dbData.technical_skills || [],
...
export function convertToOldFormat(flexibleData) {
  ...
  const spokenLanguages = flexibleData.languages || [];
  ...
  // Convert spoken languages to simple array if they're objects
  const processedSpokenLanguages = spokenLanguages
    .map((lang) => (typeof lang === "object" ? lang.language || "" : lang))
  ...
  return {
    ...
    // Combine all skills for backward compatibility
    skills: [
      ...(skills.technical || []),
      ...(skills.soft || []),
    ...
    languages: processedSpokenLanguages,
